...
```{r output}
res$design[res$design$weight > 1E-4, ]
```
...
A <- calc_A(theta = theta, FUN = peleg, design = res$design)

phi_A <- calc_phiA(design = res$design, 
                   theta = theta, FUN = peleg, tt = tt, A = A)
AI <- solve(A)
# C <- pracma::blkdiag(matrix(0), diag(1, n))
# penality <- sum(diag(C * AI * t(C)))
plot_direction_Aopt(design = res$design, tt = tt, FUN = peleg, 
